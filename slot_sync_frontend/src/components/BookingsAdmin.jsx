...
import './Bookings.css';
import { getBookedSlots, getSlotImageById, changeBookingStatus, rateSlotById } from '../api/SlotSyncApiService.js';
...
export default function Bookings() {
    ...
    async function refreshBookings() {
        try {
            const response = await getBookedSlots(authContext.id);
                        ...
                        <div key={booking.id} className="booking-item">
                            ...
                            <div className="booking-status">
                                {booking.status === 'CONFIRMED' && (
                                    <button className="status-button confirmed-2">Confirmed</button>
                                )}
